...
import { getAllOrderCountFn, getAllOrderFn, getAllOrderHistoryFn, getOneOrderFn } from "./order.queryFn"
...
export const useGetPendingOrderQuery = (status: string, size: string) => {
    return useInfiniteQuery({
        ...
        queryFn: ({pageParam = 1}) => getAllOrderFn(status, size, String(pageParam)),
        getNextPageParam: (lastPage) => {
            return lastPage.page < lastPage.totalPages ? lastPage.nextPage : undefined
        },
        initialPageParam: 1
     })
}
...
export const useGetOrderHistoryQuery = (status: string, size: string) => {
    return useInfiniteQuery({
        ...
        queryFn: ({pageParam = 1}) => getAllOrderHistoryFn(status, size, String(pageParam)),
        getNextPageParam: (lastPage) => {
            return lastPage.page < lastPage.totalPages ? lastPage.nextPage : undefined
        },
        initialPageParam: 1
