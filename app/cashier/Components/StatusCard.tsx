...
const StatusCard = () => {
    ...
    const {data: orderCount, isPending, isError } = useGetAllOrdersCount();
  return (
      ...
      <Card onClick={() => setStatus('')} className="cursor-pointer">
        <CardContent className="flex justify-between gap-4">
          <div className="flex flex-col">
            ...
            <h2 className="text-lg font-semibold text-blue-400">{orderCount?.total}</h2>
          </div>
      ...
      <Card onClick={() => setStatus('PENDING')} className="cursor-pointer">
        <CardContent className="flex justify-between gap-4">
          <div className="flex flex-col">
            ...
            <h2 className="text-lg font-semibold text-orange-300">{orderCount?.pending}</h2>
          </div>
      ...
      <Card onClick={() => setStatus('CONFIRMED')} className="cursor-pointer">
        <CardContent className="flex justify-between gap-4">
          <div className="flex flex-col">
            ...
            <h2 className="text-lg font-semibold text-red-400">{orderCount?.confirmed}</h2>
          </div>
      ...
      <Card onClick={() => setStatus('COMPLETED')} className="cursor-pointer">
        <CardContent className="flex justify-between gap-4">
          <div className="flex flex-col">
            ...
            <h2 className="text-lg font-semibold text-green-400">{orderCount?.completed}</h2>
          </div>
